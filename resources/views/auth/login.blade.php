<x-layout.auth title="Login" subtitle="Welcome back! Please sign in to your account">
    ...
    <div class="text-center mb-6">
        <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-1">Masuk</h2>
        <p class="text-sm text-gray-600">Akses dashboard Anda</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4" x-data="{ showPassword: false }">
        ...
        <div>
            <label for="nik" class="block text-sm font-medium text-gray-700 mb-1.5">
                <i class="fas fa-id-card mr-1.5 text-gray-400 text-xs"></i>NIK
            </label>
            <input id="nik" type="number" name="nik" value="{{ old('nik') }}" required autofocus
                autocomplete="username"
                class="w-full px-3 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('nik') border-red-500 @enderror"
